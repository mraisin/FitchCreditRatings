...
The dataset has multiple entries for each country. There is may be serial autocorrelation correlation within the data. A countries previous rating will almost certainly be correlated with their following rating.
...
We can see that their is a lot of missingness when we join all the features together. I attempted to impute the data from other sources but it reduced the accuracy of the models. As such, I decided to drop the observations with missing data.
...
After dropping rolling interest paymentsand omitting NAs and vaiables we still have 311 observations to train our machine learning algorithms. 
...
The above two diagrams show the distribution of the data minus missingness.  The irony is the AA rated countries share less open data. This decline in observations in the AA grade is mostly attrubutable to the AA and AA+ rated countries. The were also a significant number of AAA countries that I did not have the complete data. 
...
Linear regression has an adjusted r-sqared of .86. The high correlation between Government Dept to GDP and its three year average appears to be confusing the regression of the individual magnitude of the coefficents. However, this should not bias the overall prediction. If you add them together you get a slope of around 4. The below graphic plots the residuals on fitted values with a LOESS curve. We can see the residuals. This looks good for to satisfy the homoskedacity and non-linarity assumptions
