...
During the GFC in 2007, Australia managed to maintain its AAA credit rating and used this to guaranty the deposits in our banks. This was very important to the maintenance of the Australian economy through ensuring the availability of low yield foreign lending. 
However, Mr Jolly, head of research for one of the big for banks in Australia, wrote a recent report stating that Australia faces an upcoming Federal election with Commonwealth debt already "pushing against ratings agencies' AAA boundary". The risk to Australia of a reduction in its credit rating would be associated with increased cost to finance Governmental operations through bonds. However, how does a country maintain her credit rating when the exact method of calculation is unknown? 

My literature review indicates that statistical analysis using this economic theory was performed for S&P and Moody ratings in 1996 and later for the Big Three in 2007. In this project I will update this research for Fitch with new data and complement it by utilising machine learning.


#Introduction

A government has various ways to raises funds such as taxation, selling assets or debt. Bonds are either denominated in the local currency or a foreign currency. The bond repayments are split into principle and interest payments. The principle is paid at the maturity of the bond with scheduled interest payments. I will be looking at the credit ratings for the issuing of long term securities in the form of 10 year bonds in a sovereign's local currency.
...
Interestingly, securities are the mechanism used to 'print money': "If a central bank purchases a government security, such as a bond or treasury bill, it increases the money supply, in effect creating money." Wikipedia

I will be looking at local currency ratings which should be the apex of the credit rating compared to foreign currency credit ratings. If a sovereign's economy worsens then a counties local currency may depreciate in relation to the denomination of foreign currency securities thus increasing the risk of a default on. Namely, less capacity to pay a higher relative debt amount.

A government will engage a credit agency to rate its bonds. These ratings are used to access the risk of the bonds.  The level of risk will dictate the interest rate premium. Unfortunately, I was unable to obtain nominal bond yield data (interest on the bond) as I wished to measure the association between the yields on the bonds (premium for risk) and sovereign ratings

Credit ratings agencies are independent and are paid by the sovereign nation to provide an impartial service. As part of the arrangement the Sovereign will agree to private information sharing. Of the bid three rating agencies (S&P, Moody's, and Fitch) I will be examining Fitch's ratings as they have made their ratings from 1994-2012 [publicly available](https://www.fitchratings.com/web_content/ratings/sovereign_ratings_history.xls) and in the interest of transparency they have made an overview of their current methodology available [here](https://www.fitchratings.com/site/criteria/pfsov.html).

The below table indicates the ratings and their associated categories and grades. AAA is the prime rating. I will be encoding the ratings where AAA = 1, AA+ =2, AA = 3 and so forth. Strictly speaking there is the D/RD ratings that indicate that a country is defaulting. I have not included these as that is not a prediction but rather an indicator of what has happened.
...
The following table depicts the features I used based on Fitch's ratings criteria and their sources. I learnt from this assignment, high income nations have fewer reporting requirements making some of the data difficult to obtain. Some sovereigns, such as Argentina, have reported dubious statistics in the past and may of those have now been removed from the databases of organisations such as the World Bank, IMF and the OECD.  Country names need some massaging to link together the various databases with external agencies. I did try to input some of the missing data from the CIA Factbook (an amazing resource that could not be laid out worse) but that made the predictions worse. I assume that the CIA use different formula for calculating their statistics.

![Variables](./DATA/Variables.jpg)

It is noteworthy that where possible, Fitch prescribes using ratios I assume to remove currency distortions and facilitate year to year comparisons, failing that currency amounts are standardised to USD. 
...
Unfortunately, I was unable to get the data for foreign denominated debt for enough countries to be meaningful. This is an important statistic as foreign debt increases risk.

The nomenclature of the various idicators by the various organisations made obtaining the data somewhat challenging.

Thank you to the following platforms/open datasets as we read and wrangle the data below.
...
The dataset has multiple entries for each country. There is likely to be serial autocorrelation correlation within the data.
...
We can see that their is a lot of missingness when we join all the features together. 
...
After omitting NAs and droping rolling interest payments vaiables we still have 311 observations to train our machine learning algorithms.
...
In this section we will visualise the ratings data and the features. Then map the country ratings to the world map to look for regional effects.
...
Notice the dip at BB for the credit ratings. BBB- is the cut off for investment grade. The frequency of BB+ and BB- over BB is quite noticable.  
...
Now the distortion around BB has disappeared and the distribution is also looking more normal. 
...
The above two diagrams show the distribution of the data minus missingness.  The irony is the AA rated countries share less open data. This decline in observations in the AA grade is mostly attrubutable to the AA and AA+ rated countries. 
...
There is a clear divide around the world between South America, North Africa, Indian Subcontinent and the former USSR compared to North America, Western Europe and Australaisia. With the former largerly considered non investment grade (Junk). Without access low yeild bonds (cheap finance) how do these countries fund capital expenditures to develop? 
...
The below allows a 2D visualisation of the measure of variation each feature contributes through preinciple component analysis. These two components represent about 85% of the variation.
...
Linear regression has an adjusted r-sqared of .86. The high correlation between Government Dept to GDP and its three year average appears to be confusing the regression of the individual coefficents. This should not bias the overall prediction. If you add them together you get a slope of around 4. 
...
The gradient boosted tree algorithm has the best predictive power. When we consider the consider the gain for each of the variables we can see that the Governance feature has the highest gain. The Governance indicator is the average of the percentile rank for six indicators: 
...
To get a better understanding of governance we can break down that composite variable into it individual components and run a regression analysis.  
...
When we break governance into its six components the following are significant and have the highest slope.

*  Government Effectiveness
*  Rule of Law: Percentile Rank
*  Regulatory Quality

The above spider chart displays the average rank for each of the measures of governance for AAA, investment and non-investment grade countries.  We can see that non-investment grade countries are particularily struggling with Rule of Law, Polital Stability/Absence of Violence and Control of Corruption. Notwithstanding, on every governance measure there is an improvement as the ratings improve.   
...
Considering that there was some indicators missing and I was not working with exactly the same data as Fitch, I believe that creating an ensemble that can predict credit ratings within one notch as an encouraging for the utility of machine learning for solving real life expert problems. I was impressed how well a 'black box' algorithm like XGBoost performed on the data. 
...
There is a feature forth in place, Government Debt to GDP. High Government Debt to GDP can cause a government to 'print money' which causes inflation, reduces a currencies value which in turn can create sovereign credit risk. Debt is alright if you can afford it. If it is accompanied by high enough GDP to make a small ration then there is no problem.

Subject to more time I would be interested in applying  machine learning techniques to real bond data. If we calculate Expected Loss in a default to be equal to the product probability of a default and complement of the expected recovery amount (L = p.d *(1-recovery)) then we could train an algorithm on past performance. There is the capacity to include far more features. Furthermore, the algorith can continue to learn as new data is applied. 

This project has demonstrated that combining methods that create unbias estimates by using different techniques to real life problem will still improve our predictions - even when those methods that have less efficacy.
