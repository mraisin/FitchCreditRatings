...
The dataset has multiple entries for each country. The is likely to be serial autocorrelation correlation within the data.
...
Notice the dip at BB for the credit ratings. BBB- is the cut off for investment grade 
...
Now the distortion around BB has disappeared. The distribution is also looking more normal. 
...
The above two diagrams show the distribution of the data minus missingness.  The irony is the AA rated countries share less data openly. This decline is mostly attrubutable to the AA and AA+ rated countries. 
...
There is a clear divide around the world between South America, North Africa, Indian Subcontinent and the former USSR compared to North America, Western Europe and Australaisia. With the former largerly considered non investment grade (Junk). Without access low yeild bonds (cheap finance) how do these countries fund capital expenditures to develop? The may well be continental cluster effects within the data. 
...
The xgboost algorithm had the lowest RMSE of: `r RMSE_xgb `
...
Linear regression has an adjusted r-sqared of .86. The high correlation between Government Dept to GDP and its three year average appears to be confusing the regression of the individual coefficents. This should not bias the overall prediction.

When we blended the algorithms predictions into an ensemble we reduced the mean square error to aproximately one. This means that on average with the machine learning our prediction were within one notch of the offical Fitch rating. That is, if the prediction was BBB then on average the prediction would be within BB+ to BB-.
